...
<body>
    <div class="card-header">
        <label class="title col-md-8 col-form-label"><a href="{{ action('DestinoController@index') }}">Destinatarios</a>
            <i class="fa fa-chevron-right"></i> Añadir destinatario</label>
    </div>
    <div class="container">
        <div class="card">
            <form action="{{action('DestinoController@store')}}" method="POST" autocomplete="off">
                {{ csrf_field() }}
                @if ($errors->any())
                ...
                @endif
                <p class="form-title"><strong>Datos del destinatario</strong></p>
                <label for="nombre">
                ...
                <label for="cp" id="cod" style="display:;">
                    ...
                    <input type="number" value="{{old('cp')}}" name="cp" id="cp" max="9999" min="0"
