...
@section('content')
    ...
    <div class="card-header">
        <label class="title col-md-8 col-form-label"><b>Editar contactos de intermediario {{$intermediario->nombre}}</b></label>
    </div>
			<div class="container">
                    ...
                    @foreach ($tipoContacto as $tipo)
                        @foreach ($intermediarioContacto as $contacto)
                            @if ($tipo->idTipoContacto == $contacto->tipo)
                                    ...
                                    <a href="{{ action('IntermediarioController@delete_contact', $contacto->id)}}"><button  title="Eliminar" style="position: relative; top: 10%; left: 20%;">Eliminar</button></a>
                            @endif
                        @endforeach
                    @endforeach
                @else
               ...
               <form action="{{action('IntermediarioController@add_contact', $intermediario->cuit)}}" method="GET">
                    ...
                    <a href="{{ action('IntermediarioController@show', $intermediario->cuit)}}"><button  title="Salir" style="position: relative; top: 10%; left: 20%;">Salir</button></a>
               </div>
         </div>
      </div>
   </body>
